...
import { FaFilePdf, FaUser, FaMoneyBill, FaEdit, FaTrash } from 'react-icons/fa';
...
const MyPayBills = () => {
  ...
  if (!user) return <p className="text-center mt-20 text-lg text-gray-500">Please log in</p>;
  ...
  return (
    <div className="max-w-6xl mx-auto mt-8 px-4">
      <h1 className="text-2xl sm:text-3xl font-bold text-center mb-6 text-[#0077b6]">My Paid Bills</h1>
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        ...
        <div className="flex-1 flex items-center gap-4 bg-white rounded-lg shadow-lg p-5 min-w-[180px]">
          ...
          <div className="flex flex-col justify-center">
            ...
            <p className="text-xl sm:text-2xl font-semibold">৳{totalAmount.toLocaleString()}</p>
          </div>
        </div>
        <button
      ...
      <div className="hidden md:block overflow-x-auto rounded-lg shadow-lg bg-white">
        {data.length === 0 ? (
          <p className="text-center p-10 text-gray-500 text-sm">No payments found.</p>
        ) : (
          <table className="w-full min-w-max table-auto text-sm border-collapse">
            <thead className="bg-blue-100 sticky top-0 z-10">
              <tr>
                {["Username", "Email", "Amount", "Address", "Phone", "Date", "Actions"].map((header) => (
            ...
            <tbody>
              {data.map((bill) => (
                <tr key={bill._id} className="hover:bg-blue-50 transition">
                  ...
                  <td className="border-t px-4 py-3 text-green-700 font-semibold">৳{bill.amount}</td>
      ...
      <div className="md:hidden flex flex-col gap-4">
        {data.length === 0 ? (
          <p className="text-center p-6 text-gray-500 text-sm">No payments found.</p>
        ) : (
          data.map((bill) => (
            <div key={bill._id} className="bg-white rounded-lg shadow-lg p-4 flex flex-col gap-2">
